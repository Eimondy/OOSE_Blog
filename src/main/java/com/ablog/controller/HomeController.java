...
public class HomeController extends BaseController {
    ...
    public APIResponse comment(HttpServletRequest request, HttpServletResponse response,
        ...
        UserDomain userInfo = userService.getUserInfoById(uid);
...
//        }
        ...
        comments.setAuthor(userInfo.getScreenName());
        comments.setAuthorId(userInfo.getUid().toString());
        comments.setCid(cid);
        comments.setIp(request.getRemoteAddr());
        comments.setContent(content);
        comments.setEmail(userInfo.getEmail());

        try {
            commentService.addComment(comments);
            cookie("tale_remember_author", URLEncoder.encode(userInfo.getUsername(),"UTF-8"), 7 * 24 * 60 * 60, response);
            cookie("tale_remember_mail", URLEncoder.encode(userInfo.getEmail(),"UTF-8"), 7 * 24 * 60 * 60, response);
            // 设置对每个文章1分钟可以评论一次
            cache.hset(Types.COMMENTS_FREQUENCY.getType(),val,1,60);
            return APIResponse.success();
        } catch (Exception e) {
            return APIResponse.fail(ErrorConstant.Comment.ADD_NEW_COMMENT_FAIL + ((BusinessException)e).getErrorCode());
        }
    }
